...
- **Targeted delegation instructions** in `code-verifier`, `technical-designer(-frontend)`, `work-planner`, and `gamedev-work-planner`, tied to what each actually produces: existence and absence claims for the verifier (an absence claim needs the widest sweep, since one missed usage falsifies it), consumers of a contract before a design commits to changing it, and Target Files per task for the planners — a path missing there blocks the executor mid-task under its File Scope Constraint. The other 14 code-touching agents carry the generic form.
