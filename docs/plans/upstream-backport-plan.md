...
Register in `skills/skills-index.yaml` and the four affected `plugin.json` files.
...
- **`skill-creator` / `skill-reviewer` agent pair** — skill-authoring pipeline with a BP-001..008 pattern catalog and A/B/C grading. Directly relevant since this repo *is* a skill author, but requires first defining our own BP catalog matching `plugin-dev` conventions, and overlaps the existing `plugin-dev:skill-development` skill. Needs a scoping decision.
- **`skill-optimization` upgrade** — upstream has unique finding-IDs, a rewrite-pattern table, a 3-gate flow with defined pass conditions split across `creation-guide.md` / `review-criteria.md`, and a Split Decision rule (>400 lines → extract to `references/`, SKILL.md <250 lines). Meta-skill, so compounding value, but pairs naturally with `skill-creator`/`skill-reviewer` above.
...
- **`skills-index.yaml` duplication** — the file exists at both `skills/skills-index.yaml` and `skills/task-analyzer/references/skills-index.yaml`. Upstream keeps one copy. Two sources of truth for the same data is a maintenance risk regardless of whether they are currently in sync; decide which is canonical.
...
5. New agents (`codebase-analyzer`, `ui-analyzer`) and skill (`llm-friendly-context`) appear in all four affected `plugin.json` files and in `skills/skills-index.yaml`
